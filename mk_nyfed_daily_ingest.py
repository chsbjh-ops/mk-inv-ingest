...
#   CMDI: 인터랙티브(JS) 페이지의 'Download the data' 실제 파일 URL을 환경변수로 주입(미설정 시 건너뜀).
CMDI_URL = os.environ.get("MK_NYFED_CMDI_URL", "").strip()
...
def fetch_cmdi(start):
    if not CMDI_URL:
        log.warning("[CMDI] MK_NYFED_CMDI_URL 미설정 — 건너뜀(인터랙티브 'Download the data' 링크 지정)")
        return []
    rows = []
    try:
