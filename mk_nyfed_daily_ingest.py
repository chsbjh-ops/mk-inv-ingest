...
CMDI_URL = os.environ.get(
    "MK_NYFED_CMDI_URL",
    "https://www.newyorkfed.org/medialibrary/research/interactives/cmdi/downloads/Market%20CMDI.xlsx",
).strip()
...
CMDI_MAP = [  # (MAST_ID, SERIES_CD, SERIES_NM, 컬럼 후보 키워드(우선순위))
    (7, "CMDI_MKT", "회사채 시장 디스트레스(CMDI 전체)", ["market", "aggregate", "agg"]),
    (8, "CMDI_IG",  "회사채 시장 디스트레스(CMDI IG)",   ["investment", "inv grade", "ig"]),
    (9, "CMDI_HY",  "회사채 시장 디스트레스(CMDI HY)",   ["high yield", "high", "hy"]),
]
...
def fetch_cmdi(start):
    ...
    try:
        ...
        dcol = next((c for c in df.columns if "date" in c or c in ("month", "observation_date")),
                    df.columns[0])
        df[dcol] = pd.to_datetime(df[dcol], errors="coerce")
        df = df[df[dcol].notna() & (df[dcol] >= pd.to_datetime(start))]
        used = set()
        for mid, scd, snm, keys in CMDI_MAP:                 # 우선순위·중복방지 매칭
            col = None
            for k in keys:
                col = next((c for c in df.columns
                            if c != dcol and c not in used and k in c), None)
                if col:
                    break
            if not col:
                log.warning("[CMDI] 컬럼 매칭 실패 %s (가용:%s)", scd, list(df.columns)); continue
            used.add(col)
            for _, x in df[[dcol, col]].dropna().iterrows():
                rows.append({"MAST_ID": mid, "SERIES_CD": scd, "SERIES_NM": snm,
                             "TD": _td(x[dcol]), "VALUE": float(x[col])})
        log.info("[CMDI] %d행 (열매칭: %s)", len(rows), sorted(used))
    except Exception as e:
        log.error("[CMDI] 실패(URL/포맷 확인): %s", e)
    return rows
