...
def _read_table(content, preview_tag=None):
    """헤더가 상단 설명행에 가려진 경우 키워드 포함 행을 헤더로 자동탐지."""
    raw = pd.read_excel(io.BytesIO(content), header=None)
    if preview_tag:                                        # 구조 진단용 상위행 덤프
        log.info("[CMDI] %s 원시 상위행(최대 8행×8열):", preview_tag)
        for i in range(min(8, len(raw))):
            vals = [str(v)[:20] for v in raw.iloc[i].tolist()[:8]]
