...
#   (MAST_ID, SERIES_CD, SERIES_NM, 열이름 키워드(우선순위))  — 'Index Data' 시트의 열
CMDI_MAP = [
    (7, "CMDI_MKT", "회사채 시장 디스트레스(CMDI 전체)", ["market", "aggregate", "agg"]),
    (8, "CMDI_IG",  "회사채 시장 디스트레스(CMDI IG)",   ["investment", "grade", "ig"]),
    (9, "CMDI_HY",  "회사채 시장 디스트레스(CMDI HY)",   ["high yield", "high", "yield", "hy"]),
]
...
def _headerize(raw):
    """헤더가 상단 설명행에 가려진 시트를 값 기반으로 정규화.
       날짜 최다 열의 첫 데이터행 윗행을 헤더로 사용 → (df, 날짜열명)."""
    best_j, best = None, 0
    for j in raw.columns:
        c = pd.to_datetime(raw[j], errors="coerce").notna().sum()
        if c > best:
            best, best_j = c, j
    if best_j is None or best < 12:
        return None, None
    first = int(pd.to_datetime(raw[best_j], errors="coerce").notna().idxmax())
    hdr = max(first - 1, 0)
    df = raw.iloc[hdr:].copy()
    df.columns = [str(x).strip().lower() for x in df.iloc[0].tolist()]
    df = df.iloc[1:].dropna(axis=1, how="all").reset_index(drop=True)
    dname = next((c for c in df.columns
                  if pd.to_datetime(df[c], errors="coerce").notna().mean() > 0.7), None)
    return df, dname

def _pick_data_sheet(sheets):
    """날짜·숫자가 가장 많은 시트를 데이터 시트로 선택('Index Data' 등)."""
    best_name, best_score = None, -1
    for name, raw in sheets.items():
        dscore = max((pd.to_datetime(raw[j], errors="coerce").notna().sum()
                      for j in raw.columns), default=0)
        if dscore > best_score:
            best_score, best_name = dscore, name
    return best_name
...
def fetch_cmdi(start):
    ...
    name = _pick_data_sheet(sheets)
    df, dname = _headerize(sheets[name])
    if df is None or dname is None:
        log.warning("[CMDI] 데이터 시트 인식 실패 (시트:%s)", name); return []
    log.info("[CMDI] 데이터 시트 '%s' 열: %s", name, list(df.columns))

    s0 = pd.to_datetime(start)
    dates = pd.to_datetime(df[dname], errors="coerce")
    rows, used = [], {dname}
    for mid, scd, snm, keys in CMDI_MAP:
        col = None
        for k in keys:                                     # 우선순위·중복방지
            col = next((c for c in df.columns if c not in used and k in c), None)
            if col:
                break
        if col is None and mid == 7:                       # 시장: 남은 숫자열로 폴백
            cand = [(pd.to_numeric(df[c], errors="coerce").notna().sum(), c)
                    for c in df.columns if c not in used]
            col = max(cand)[1] if cand and max(cand)[0] >= 12 else None
        if col is None:
            log.warning("[CMDI] 열매칭 실패 %s (가용:%s)", scd, list(df.columns)); continue
        used.add(col)
        ...
        for d, v in zip(dates, vals):
            if pd.notna(d) and pd.notna(v) and d >= s0:
                rows.append({"MAST_ID": mid, "SERIES_CD": scd, "SERIES_NM": snm,
                             "TD": _td(d), "VALUE": float(v)}); n += 1
        log.info("[CMDI] %s ← '%s' %d행", scd, col, n)
