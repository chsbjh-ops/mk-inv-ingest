...
#   dp     : 가격 반올림 소수 자릿수
# -----------------------------------------------------------------------------
# MAST_ID: 순차 1~24  (BM ETF 1~11 → 지수 12~18 → 환율·변동성·원자재 19~24)
INDICATORS = [
    # ── BM 벤치마크 ETF (1~11) ──────────────────────────────────────────────
    # MAST_ID, series_cd,       series_nm,                                     use,   yf,         vol, dp
    (1,  "VOO",          "Vanguard S&P 500 ETF",                        "BM", "VOO",     "Y", 2),
    (2,  "VEA",          "Vanguard FTSE Developed Markets ETF",         "BM", "VEA",     "Y", 2),
    (3,  "VWO",          "Vanguard FTSE Emerging Markets ETF",          "BM", "VWO",     "Y", 2),
    (4,  "BIL",          "SPDR Bloomberg 1-3 Month T-Bill ETF",         "BM", "BIL",     "Y", 2),
    (5,  "GOVT",         "iShares U.S. Treasury Bond ETF",              "BM", "GOVT",    "Y", 2),
    (6,  "BWX",          "SPDR Bloomberg International Treasury Bond ETF","BM", "BWX",    "Y", 2),
    (7,  "QLTA",         "iShares Aaa - A Rated Corporate Bond ETF",    "BM", "QLTA",    "Y", 2),
    (8,  "IEAC.AS",      "iShares Core € Corp Bond UCITS ETF",          "BM", "IEAC.AS", "Y", 2),
    (9,  "MBB",          "iShares MBS ETF",                             "BM", "MBB",     "Y", 2),
    (10, "VMBS",         "Vanguard Mortgage-Backed Securities ETF",     "BM", "VMBS",    "Y", 2),
    (11, "IGF",          "iShares Global Infrastructure ETF",           "BM", "IGF",     "Y", 2),
    # ── INV 시장지표 — 지수 (12~18) ─────────────────────────────────────────
    (12, "KS11",         "코스피(KOSPI)",                 "INV", "^KS11",     "Y", 2),
    (13, "KS200",        "코스피200",                     "INV", "^KS200",    "Y", 2),
    (14, "KQ11",         "코스닥(KOSDAQ)",                "INV", "^KQ11",     "Y", 2),
    (15, "DJI",          "다우존스 산업평균",             "INV", "^DJI",      "Y", 2),
    (16, "SP500(SPX)",   "S&P500",                        "INV", "^GSPC",     "N", 2),
    (17, "NASDAQ(IXIC)", "나스닥 종합",                   "INV", "^IXIC",     "Y", 2),
    (18, "SOX",          "필라델피아 반도체",             "INV", "^SOX",      "N", 2),
    # ── INV 시장지표 — 환율·변동성·원자재 (19~24) ──────────────────────────
    (19, "USD",          "원/달러 환율",                  "INV", "KRW=X",     "N", 2),
    (20, "CNY",          "원/위안 환율",                  "INV", "CNYKRW=X",  "N", 2),
    (21, "JPY",          "원/엔 환율",                    "INV", "JPYKRW=X",  "N", 4),
...
DATA_COLS = ["MAST_ID", "SERIES_CD", "SERIES_NM", "USE_GB", "TD",
             ...
             "VOLUME", "CHG_RT", "INSERT_SQL"]

START_DEFAULT = "2026-05-01"
SLEEP_BETWEEN = 0.3


# --- 서버 수기 INSERT 용 SQL 생성 (MS-SQL) -----------------------------------
#   대상: MK_YFINANCE_DATA (OHLC + 수정종가). USE_GB 는 MAST 에 있으므로 제외.
def _sv(v):   # 숫자 또는 NULL
    return "NULL" if v is None or (isinstance(v, float) and pd.isna(v)) else f"{v}"


def _ss(v):   # 문자열 또는 NULL
    return "NULL" if v is None else "'" + str(v).replace("'", "''") + "'"


def build_insert(d) -> str:
    return ("INSERT INTO MK_YFINANCE_DATA "
            "(MAST_ID, TD, CLOSE_PRC, ADJ_CLOSE_PRC, OPEN_PRC, HIGH_PRC, LOW_PRC, "
            "VOLUME, CHG_RT, REG_DT, REMARK) VALUES("
            f"{d['MAST_ID']},'{d['TD']}',{_sv(d['CLOSE_PRC'])},{_sv(d['ADJ_CLOSE_PRC'])},"
            f"{_sv(d['OPEN_PRC'])},{_sv(d['HIGH_PRC'])},{_sv(d['LOW_PRC'])},"
            f"{_ss(d['VOLUME'])},{_sv(d['CHG_RT'])},GETDATE(),NULL);")
...
def collect_one(meta, start, end) -> list[dict]:
    ...
    try:
        ...
        for idx, r in df.iterrows():
            ...
            row["INSERT_SQL"] = build_insert(row)   # 서버 수기 INSERT 용
            rows.append(row)
            prev_close = close
